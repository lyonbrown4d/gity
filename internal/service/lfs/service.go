...
type DownloadObject struct {
	Object  entity.ProjectLFSObject
	Content []byte
}
...
func (s *Service) UploadObject(ctx context.Context, projectID int64, oid string, content []byte) (entity.ProjectLFSObject, error) {
	project, err := s.projectRepo.GetByID(ctx, projectID)
	if err != nil {
		return entity.ProjectLFSObject{}, httpx.NewError(http.StatusNotFound, "project not found", err)
	}
	oid = strings.TrimSpace(oid)
	if err := validateOID(oid); err != nil {
		return entity.ProjectLFSObject{}, httpx.NewError(http.StatusBadRequest, "invalid lfs oid", err)
	}
	...
	if err := s.storage.SaveObject(ctx, storageKey, content, "application/octet-stream"); err != nil {
		return entity.ProjectLFSObject{}, err
	}
	record, err := s.objectRepo.GetByProjectAndOID(ctx, projectID, oid)
	if err == nil {
...
func (s *Service) listLockEntities(ctx context.Context, projectID int64, input LockListInput) ([]entity.ProjectLFSLock, string, error) {
	if _, err := s.projectRepo.GetByID(ctx, projectID); err != nil {
	...
	if strings.TrimSpace(input.ID) != "" {
		...
		if err != nil {
			if err == dbxrepo.ErrNotFound {
				return []entity.ProjectLFSLock{}, "", nil
			}
			return nil, "", err
		}
		return []entity.ProjectLFSLock{item}, "", nil
	}
...
func (s *Service) buildLockView(ctx context.Context, item entity.ProjectLFSLock) (LockView, error) {
	owner, err := s.userRepo.GetByID(ctx, item.OwnerUserID)
	if err != nil {
...
func buildLockView(item entity.ProjectLFSLock, owner entity.User) LockView {
