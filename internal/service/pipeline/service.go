...
type PipelineView struct {
	Pipeline entity.ProjectPipeline `json:"pipeline"`
	Spec     plandsl.PipelineSpec   `json:"spec,omitempty"`
	Jobs     []PipelineJobView      `json:"jobs,omitempty"`
}

type PipelineJobView struct {
	PipelineJob entity.ProjectPipelineJob `json:"pipeline_job"`
	ProjectJob  entity.ProjectJob         `json:"project_job"`
...
func (s *Service) ListPipelines(ctx context.Context, projectID int64) ([]entity.ProjectPipeline, error) {
...
func (s *Service) loadPipeline(ctx context.Context, projectID int64, pipelineID int64) (entity.ProjectPipeline, error) {
	if _, err := s.projectRepo.GetByID(ctx, projectID); err != nil {
		return entity.ProjectPipeline{}, httpx.NewError(http.StatusNotFound, "project not found", err)
	}
	item, err := s.pipelineRepo.GetByProjectAndID(ctx, projectID, pipelineID)
	if err != nil {
		if err == dbxrepo.ErrNotFound {
			return entity.ProjectPipeline{}, httpx.NewError(http.StatusNotFound, "project pipeline not found", err)
		}
...
func (s *Service) refreshPipelineState(ctx context.Context, projectID int64, pipelineID int64) (entity.ProjectPipeline, error) {
	pipeline, err := s.loadPipeline(ctx, projectID, pipelineID)
	if err != nil {
		return entity.ProjectPipeline{}, err
	}
	...
	jobs := make(map[string]entity.ProjectJob, items.Len())
	for _, item := range items.Values() {
		job, err := s.jobRepo.GetByID(ctx, item.ProjectJobID)
		if err != nil {
			return entity.ProjectPipeline{}, err
...
func (s *Service) releaseReadyJobs(ctx context.Context, items []entity.ProjectPipelineJob, jobs map[string]entity.ProjectJob) error {
	now := time.Now().UTC()
...
func (s *Service) cancelPendingJobs(ctx context.Context, jobs map[string]entity.ProjectJob) error {
...
func pipelineJobStatus(job entity.ProjectJob, needs []string) string {
...
func pipelineStatus(jobs map[string]entity.ProjectJob) string {
	if len(jobs) == 0 {
...
func needsSucceeded(needs []string, jobs map[string]entity.ProjectJob) bool {
