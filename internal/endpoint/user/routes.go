...
func RegisterRoutes(server httpx.ServerRuntime, service *userservice.Service, authRuntime *platformauth.Runtime) {
...
func (e *Endpoint) Register(registrar httpx.Registrar) {
	...
	listUsers := func(ctx context.Context, in *usersInput) (*userOutput, error) {
		...
		items.Range(func(_ int, item entity.User) bool {
...
func currentUser(ctx context.Context, service *userservice.Service, authRuntime *platformauth.Runtime, authorization string) (entity.User, error) {
	principal, ok, err := authRuntime.AuthenticateHeader(ctx, authorization)
	if err != nil {
		return entity.User{}, err
	}
	if !ok {
		return entity.User{}, httpx.NewError(401, "authentication required", nil)
	}
	return service.GetByID(ctx, principal.UserID)
}

func toUserView(item entity.User) userView {
	return userView{
