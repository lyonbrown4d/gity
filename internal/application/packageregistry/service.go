...
func (s *Service) UploadFile(ctx context.Context, projectID int64, input UploadFileInput) (packagedomain.ProjectPackageFile, error) {
	...
	if err != nil {
		return packagedomain.ProjectPackageFile{}, httpx.NewError(http.StatusNotFound, "project not found", err)
	}
...
func (s *Service) loadPackage(ctx context.Context, projectID int64, packageID int64) (packagedomain.ProjectPackage, error) {
	if _, err := s.projectRepo.GetByID(ctx, projectID); err != nil {
		return packagedomain.ProjectPackage{}, httpx.NewError(http.StatusNotFound, "project not found", err)
	}
	pkg, err := s.packageRepo.GetByID(ctx, packageID)
	if err != nil {
		if err == dbxrepo.ErrNotFound {
			return packagedomain.ProjectPackage{}, httpx.NewError(http.StatusNotFound, "package not found", err)
		}
		return packagedomain.ProjectPackage{}, err
	}
	if pkg.ProjectID != projectID {
		return packagedomain.ProjectPackage{}, httpx.NewError(http.StatusNotFound, "package not found", nil)
	}
	return pkg, nil
}
