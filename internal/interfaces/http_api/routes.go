...
func (r route[I, O]) register(group *httpx.Group) {
	handler := adaptAppError(r.handler)
	if len(r.config.policies) == 0 {
		httpx.MustGroupRoute(group, r.method, r.path, handler, r.config.operations...)
		return
	}
	...
	httpx.MustGroupRouteWithPolicies(group, r.method, r.path, handler, policies...)
}

func adaptAppError[I, O any](handler httpx.TypedHandler[I, O]) httpx.TypedHandler[I, O] {
	return func(ctx context.Context, input *I) (*O, error) {
		...
		if appErr.Err != nil {
			return output, httpx.NewError(appErr.Status, appErr.Message, appErr.Err)
		}
		return output, httpx.NewError(appErr.Status, appErr.Message)
	}
}
