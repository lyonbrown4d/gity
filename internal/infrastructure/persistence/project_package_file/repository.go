...
func (r *Repository) ListByVersionID(ctx context.Context, versionID int64) (*collectionx.List[packagedomain.ProjectPackageFile], error) {
	query := querydsl.Select(dbschema.ProjectPackageFileSchema.AllColumns().Values()...).From(dbschema.ProjectPackageFileSchema).Where(dbschema.ProjectPackageFileSchema.ProjectPackageVersionID.Eq(versionID)).OrderBy(dbschema.ProjectPackageFileSchema.ID.Desc())
...
func (r *Repository) ListByVersionIDs(ctx context.Context, versionIDs ...int64) (*collectionx.List[packagedomain.ProjectPackageFile], error) {
	...
	query := querydsl.Select(dbschema.ProjectPackageFileSchema.AllColumns().Values()...).From(dbschema.ProjectPackageFileSchema).Where(dbschema.ProjectPackageFileSchema.ProjectPackageVersionID.In(versionIDs...)).OrderBy(dbschema.ProjectPackageFileSchema.ProjectPackageVersionID.Desc(), dbschema.ProjectPackageFileSchema.ID.Desc())
	return persistence.Many(r.base.List(ctx, query))
}
